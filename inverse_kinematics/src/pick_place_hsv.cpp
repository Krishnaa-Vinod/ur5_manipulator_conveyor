...
int main(int argc, char** argv)
{
  ...
  auto node = rclcpp::Node::make_shared("pick_place_hsv");
  ...
  // Subscribe to pixel_to_position topic
  auto sub = node->create_subscription<geometry_msgs::msg::PointStamped>(
    "/object_position_world", 10,
    [&](const geometry_msgs::msg::PointStamped::SharedPtr msg) {
      if (!received) {
        x_pos = msg->point.x;
        y_pos = msg->point.y;
        received = true;
        RCLCPP_INFO(log, "Received position: x=%.3f, y=%.3f", x_pos, y_pos);
      }
    }
  );
  ...
  RCLCPP_INFO(log, "Waiting for initial position...");
  while (rclcpp::ok() && !received) {
    std::this_thread::sleep_for(100ms);
  }

  // Prepare common orientation
  tf2::Quaternion q; 
  q.setRPY(-PI, 0, -PI/2);
